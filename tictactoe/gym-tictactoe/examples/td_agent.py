...
@click.option(
    ...
    default=EPISODE_CNT,
    show_default=True,
    help="Learn episode count.",
)
@click.option(
    "-b",
    "--bench-episode",
    "max_bench_episode",
    default=BENCH_EPISODE_CNT,
    show_default=True,
    help="Bench " "episode count.",
)
@click.option(
    "-e",
    "--epsilon",
...
@click.option(
    "-f", "--model-file", default=MODEL_FILE, show_default=True, help="Model file name."
)
@click.option(
    "-n",
    "--show-number",
...
@click.option(
    "-q",
    "--quality",
    type=click.Choice(["high", "mid", "low"]),
    default="mid",
    show_default=True,
    help="Grid search" " quality.",
)
@click.option(
    "-r",
    "--reproduce-test",
    ...
    default=3,
    show_default=True,
    help="Reproducibility test count.",
)
    ...
    for idx, row in top10_df.iterrows():
        ...
        for i in range(rtest_cnt):
            ...
            with NamedTemporaryFile() as tmp:
                res = _learnbench(
                    learn_episode, BENCH_EPISODE_CNT, epsilon, alpha, tmp.name, False
                )
            res = json.loads(res)
