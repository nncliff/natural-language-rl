...
from env.env import MazeEnv, describe_observation, describe_observation_give_position, illegal_penalty_reward, illegal_penalty_diff_scale, manhatten_actions, standard_reward, describe_observation_only_walls
from env.mazes import double_t_maze_optimal_directions, maze2d_umaze, double_t_maze
import numpy as np
from env.environment import Text
...
def maze_solver_partrandom(maze: np.ndarray, goal_positions: List[Tuple[int, int]], prob=0.5) -> Dict[Tuple[int, int], str]:
    maze = maze.tolist()
    assert len(maze) > 0 and len(maze[0]) > 0, 'maze must be non-zero in area'
    assert all([maze[goal_pos[0]][goal_pos[1]] == 1 for goal_pos in goal_positions]), 'goal pos must be 1'
    move_mapping = {
    ...
    while len(queue) > 0:
        ...
        for dx, dy in directions:
            ...
            if np.random.random()<prob:
                ...
                optimal_policy[new_pos] = np.random.choice(list(move_mapping.values()))
    return optimal_policy